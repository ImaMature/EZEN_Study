...
public class GradeTable_TreeSet {
	/*
	...
	public static void main(String[] args) {
		...
		TreeSet<Students_TreeSet> studentList = new TreeSet<>();
			...
			System.out.println("1. 입력하기 2. 성적 출력 3. 초기화 4. 종료"); int ch = scanner.nextInt();
				if(ch == 1 ) {
					try{
						for(int i =0; i<5; i++) {
							...
							Students_TreeSet students = new Students_TreeSet(num, name, kor, eng, math);
							
							//총점
							int sum = (kor+eng+math);
							students.setTotal(sum);
							int sw = 1;
							for(Students_TreeSet c : studentList) {
								sw++;
								students.setRank(sw);}
				...
				if(ch==2) {
					...
					try {
							...
							Iterator<Students_TreeSet> iterator = studentList.iterator();
							
							while(iterator.hasNext()) {
							for(Students_TreeSet s:studentList) {
