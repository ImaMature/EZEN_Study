...
public class Book {
	...
	private String b_ISBN;	//book_ISBN
	private String b_name;
	private boolean b_rental;
	private String m_id;	//로그인한 회원정보가 입력되면 member클래스와 여기로 입력됨
	...
	//3. 메소드 [동작] [CRUD : 생성(Create), 읽기(Read), 갱신(Update), 삭제(Delete) ]
		// 2) 도서등록 메소드
		 public boolean book_add() {
			// 입력받기
			System.out.println("-------------------도서 등록 페이지------------------");
			System.out.println("도서코드 : ");		String b_ISBN = BookApplication.scanner.next(); //외부 클래스의 scanner 사용하는법																 
					// 도서코드 중복체크
			...
			// 객체생성		// 도서대여여부 초기값 => true 		// 대여한 회원 초기값 => null		
			Book book = new Book (b_ISBN, b_name, true, null); // 아직 안빌려서 null
			// 객체배열저장
		...
		// 6) ISBN 체크 메소드
		 	// [ 체크할 대상을 인수로 받아서 배열내 존재하면 true 아니면 false ]
		...
		// 3) 도서목록 메소드
		 public void book_list( ) {
			...
			for (int i= 0; i<BookApplication.books.length; i++) {
				...
				System.out.println(book.getB_ISBN() + "\t" + book.getB_name() + "\t" + book.getB_rental()+"\t"+book.m_id);
			}
			return;
		 } // void는 return 생략 가능, 반환타입이 없기 때문
		...
		// 5) 도서반납 메소드
		 public boolean book_return(String loginid) {
			...
			// 1. 현재 대여중인 책들 찾기 
			for ( int i=0; i<BookApplication.books.length; i++)	 {
				if (BookApplication.books[i] != null && !BookApplication.books[i].getB_rental()) { //대여회원이 null아니고, 책과 대여한 책이 같지 않을때
					//! : 부정 ( true => false )
					// 2. 입력받은 도서코드
